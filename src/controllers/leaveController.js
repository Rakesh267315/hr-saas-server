...
exports.apply = async (req, res) => {
  try {
    ...
    if (balanceCol) {
      const balance = parseInt(empRes.rows[0][balanceCol]);
    ...
    const r = await pool.query(
      ...
      [employeeId, leaveType, startDate, endDate, totalDays, reason, isHalfDay||false]
    );
...
exports.getAll = async (req, res) => {
  try {
    ...
    const [rows, countRow] = await Promise.all([
      pool.query(`SELECT ${leaveSelect} ${where} ORDER BY l.created_at DESC LIMIT $${params.length+1} OFFSET $${params.length+2}`, [...params, limit, offset]),
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [status, req.user.id, rejectionReason||null, req.params.id]
    );
    ...
    if (status === 'approved') {
      ...
      await Promise.all(days.map((day) => {
        ...
        return pool.query(
          `INSERT INTO attendance (employee_id,date,status) VALUES ($1,$2,'on_leave') ON CONFLICT (employee_id,date) DO UPDATE SET status='on_leave'`,
          [leave.employee_id, d]
        );
      }));
    }
...
exports.cancel = async (req, res) => {
  try {
    ...
    if (!leave) return res.status(404).json({ success: false, message: 'Leave not found' });
    if (!['pending','approved'].includes(leave.status)) return res.status(400).json({ success: false, message: 'Cannot cancel this leave' });
...
exports.getBalance = async (req, res) => {
  try {
    ...
    const e = r.rows[0];
    res.json({ success: true, data: { annual: e.leave_annual, sick: e.leave_sick, casual: e.leave_casual, maternity: e.leave_maternity, unpaid: e.leave_unpaid } });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
